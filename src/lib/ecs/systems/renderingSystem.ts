...
import { Engine } from "..";
import { ElementPosition, Tool } from "@/types/elements";
...
export class RenderingSystem implements System {
    ...
    draw() {
        ...
        for (const entity of this.engine.entities.values()) {
            ...
            if (positionC && typeC) {
                const { type, position } = { ...positionC, ...typeC }
                switch (type) {
                    case 'POINTER':
                        this.renderArrow(ctx, position)
                        break
                    case 'PERSON':
                        this.renderPerson(ctx, position)
                        break
                    case 'LOCATION':
                        break
                }
    ...
    private renderPerson(ctx: CanvasRenderingContext2D, position: ElementPosition) {
        ctx.fillStyle = 'red'
